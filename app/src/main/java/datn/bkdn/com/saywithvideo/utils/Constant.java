package datn.bkdn.com.saywithvideo.utils;

import android.os.Environment;

public class Constant {
    public static final String VIDEO = "SayWithVideo/Video/";
    public static final String AUDIO = "SayWithVideo/Audio/";
    public static final String DIRECTORY_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/";
    public static final String VIDEO_DIRECTORY_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + VIDEO;
    public static final String FIREBASE_ROOT = "https://saywithvideo.firebaseio.com/";
}
